...
function parsingTable(){
	...
	for(var i=0;i<rows;i++){//for each head
		for(var x=0;x<productions[i].length;x++){//for each production of that head
			console.log("i="+i+"\tx="+x);
			var first;
			if(!isNaN(parseInt(productions[i][x][0])))
				first=productions[i][x][0];
			else if(productions[i][x][0].toUpperCase()==productions[i][x][0])
				first=getFirst(productions[i][x][0]);//get first for that prodcution
			else
				first=productions[i][x][0];
			...
			for(var j=0;j<first.length;j++){
				for(var k=0;k<cols;k++){
					if(first[j]==table.terms[k]){
						table.values[i][k]=table.heads[i]+"->"+productions[i][x];
					}
				}
				if(first[j]=="ε"){
					var follow=getFollow(table.heads[i]);
					for(var r=0;r<follow.length;r++){
						for(var y=0;y<cols;y++){
							if(follow[r]==table.terms[y]){
								table.values[i][y]=table.heads[i]+"->"+productions[i][x];
							}
							if(follow[r]=="ε"){
								table.values[i][cols-1]=table.heads[i]+"->"+productions[i][x];
							}
						}
					}
				}
